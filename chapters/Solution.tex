...
\chapter{Optimistic Concurrency Control with Snapshot Isolation on Semantic Related Group}
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}
		\STATE {\textbf{init:} snapshot.clear, restart = \TRUE, try = 0, path = operation.src}
		...
		\WHILE{restart \AND try $<$ 5}
			...
			\STATE {tx.lockMode(Read\_Committed)}
			\STATE {tx.snapshot = resolve\_semantic\_related\_group(path)}
			\STATE {tx.snapshot\_copy = tx.snapshot}
			...
			\STATE {tx.lockMode(Shared)}
			\STATE {relatedRows\_DataBase = batchRead\_Database(tx.snapshot\_copy)}
			\STATE {tx.lockMode(Exclusive)}
			\STATE {modifiedRows\_DataBase = batchRead\_Database(tx.snapshot\_copy)}
			\IF{versionCompare(relatedRows\_DataBase, modifiedRows\_DataBase, tx.snapshot\_copy) $==$ \TRUE}
